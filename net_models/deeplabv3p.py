...
from net_models.xception import xception_65


def deeplabv3p(input_op, num_classes, training, output_stride=16, batch_norm_decay=0.9997):
    net = input_op
    with slim.arg_scope(nets.resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
        net, end_points = xception_65(net, num_classes=None,
                            is_training=training, output_stride=16, global_pool=False)
        base_model = 'xception_65'
            ...
            with tf.contrib.slim.arg_scope(nets.resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
                with slim.arg_scope([slim.batch_norm], is_training=training):
                    with tf.variable_scope("low_level_features"):
                        low_level_features = end_points[base_model + '/entry_flow/block1/unit_1/xception_module/separable_conv3_pointwise']
