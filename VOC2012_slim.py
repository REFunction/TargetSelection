...
class VOC2012:
    def __init__(self, root_path='./VOC2012/', aug_path='SegmentationClassAug/', image_size=(224, 224),
        ...
        self.instance_label_path = self.root_path + 'SegmentationObject/'
    ...
    def get_batch_aug(self, batch_size):
        ...
        #end = min(self.aug_location + batch_size, 1452)
        ...
        #self.aug_location = (self.aug_location + batch_size) % 1452
        if end - start != batch_size:
        ...
        for i in range(batch_size):
            ...
            batch_labels.append(label)
        return batch_images, batch_labels
    def get_batch_train_instance(self, batch_size=8):
        if hasattr(self, 'train_location') == False:
            self.train_location = 0
        end = min(self.train_location + batch_size, len(self.train_names))
        start = self.train_location
        ...
        for i in range(batch_size):
            ...
            if self.image_size:
                image = cv2.resize(image, self.image_size)
            label = np.array(Image.open(self.instance_label_path + batch_images_names[i] + '.png'))
            label[label == 255] = 0
            if self.image_size:
                label = cv2.resize(label, self.image_size, interpolation=cv2.INTER_NEAREST)
            batch_images.append(image)
...
if __name__ == '__main__':
    voc2012 = VOC2012('h:/VOC2012/', 'h:/VOC2012/SegmentationClassAug/', image_size=None)
    total = 0
    for i in range(len(voc2012.train_names)):
        a, b = voc2012.get_batch_train_instance(1)
        masks = resolve_mask(b[0], 50)
        total = total + len(masks)
        print(total)
    print(total)